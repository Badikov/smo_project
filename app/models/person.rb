...
class Person < ActiveRecord::Base
  ...
  validates :fam, :im, :ot, :w, :c_oksm, :status, :dr, 
            :presence => {:message => "Не должно быть пустым."}
  validates :fam, :im, :ot, 
            :length => { :maximum => 40, :too_long => "%{count} символов это максимум возможного." }
  validates :phone, 
            :length => { :maximum => 40, :too_long => "%{count} символов это максимум возможного." }, :allow_blank => true
  validates :c_oksm, 
            :length => { :is => 3 }
  validates :ss, 
            :length => { :is => 14 }, :allow_blank => true
  validates :email, 
            :length => { :maximum => 50, :too_long => "%{count} символов это максимум возможного." }, :allow_blank => true
  validates :ss, 
