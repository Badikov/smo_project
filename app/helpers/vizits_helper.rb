...
module VizitsHelper
  ...
  def vibor_zamena_smo
    vibor_arey = [["первичный выбор СМО",1],
                  ["замена СМО в соответствии с правом замены раз в год",2],
		              ["замена СМО в связи со сменой места жительства застрахованного",3],
                  ["замена СМО в связи с прекращением действия договора",4]]
    return vibor_arey
  end
  def prichina_zameni_polisa
    r_arey = [["изменение реквизитов",1],
  ...
  def forma_polisa
    f_arey = [["не требует изготовления полиса",0],
              ["бумажный бланк",1],
              ["пластиковая карта",2],
              ["в составе УЭК",3]]
    return f_arey
